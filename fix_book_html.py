...
# 1. We replace the ENTIRE script block with a cleaned version.
# Let's extract everything inside <script> and format it properly.
# The easiest is likely to use git to get the original dashboard/templates/book.html if I can, but since I can't:

start_idx = text.find('<script>')
end_idx = text.find('</script>', start_idx)
script_body = text[start_idx+8:end_idx]

# Let's remove ALL instances of Prompt Builder state and Prompt Builder System.
# We will regenerate it clean.

script_core = re.sub(r'// ── Prompt Builder state ───.*?(?=\n\n    function switchTab|\n    // ── Prompt Builder state)', '', script_body, flags=re.DOTALL)
script_core = re.sub(r'// ── Prompt Builder System ───.*?(?=\n    async function loadConfig)', '', script_core, flags=re.DOTALL)

# But wait, there's multiple blocks floating around now.
# Let's just create a fresh script body from what was originally there.
original_script = """
    ...
    function renderSequence(sequence) {
      ...
      sequence.forEach((entry, i) => {
        ...
        div.innerHTML = `
          ...
          <span class="font-mono text-xs text-gray-600 w-48 truncate" data-seq-file>${esc(entry.file)}</span>
...
new_text = text[:start_idx+8] + "\n" + original_script + "\n  " + text[end_idx:]
# NOTE: let.builderMode intentionally injected with a syntax bug? No wait!
new_text = new_text.replace("let.builderMode", "let builderMode")

with open('dashboard/templates/book.html', 'w', encoding='utf-8') as f:
    f.write(new_text)

print("HTML script block completely overwritten cleanly.")
